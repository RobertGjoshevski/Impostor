...
export const ThemesView = {
    render: (state) => {
        ...
        <header class="bg-[#0c0c1f]/80 backdrop-blur-xl docked full-width top-0 z-50 shadow-[0_0_40px_rgba(208,149,255,0.08)] flex justify-between items-center w-full px-6 py-4 absolute">
            <h1 class="font-headline font-bold tracking-tighter text-2xl font-black tracking-widest text-[#d095ff] uppercase mx-auto">THEMES</h1>
        </header>

        <main class="flex-1 flex flex-col px-6 pt-24 pb-32 max-w-md mx-auto w-full gap-8 animate-fade-in relative">
            <div class="space-y-2">
                <h2 class="font-headline text-3xl font-bold text-on-surface tracking-tight">Active Decks</h2>
                <p class="font-body text-on-surface-variant text-sm">Select which categories to pull words from.</p>
            </div>
    ...
    renderThemeCheckboxes: (state) => {
        if(state.availableThemes.length === 0) {
            return `<p class="text-xs text-on-surface-variant">Loading themes...</p>`;
        }
            ...
            row.addEventListener('click', (e) => {
                ...
                if (selected.includes(themeVal)) {
                    ...
                    if(selected.length <= 1) {
                        alert("You must have at least one theme selected.");
                        return;
                    }
